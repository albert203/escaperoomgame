...
Shader "Distant Lands/Cozy/Stylized Clouds Ghibli Mobile"
{
	...
	SubShader
	{
		...
		Tags { "RenderPipeline"="UniversalPipeline" "RenderType"="Opaque" "Queue"="Transparent-50" }
		...
		Pass
		{
			...
			CBUFFER_END
					...
					float2 voronoihash35_g43( float2 p )
					{
					...
					float voronoi35_g43( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash35_g43( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
								float d = 0.5 * dot( r, r );
						 		if( d<F1 ) {
						 			F2 = F1;
					...
					float2 voronoihash13_g43( float2 p )
					{
					...
					float voronoi13_g43( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash13_g43( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
								float d = 0.5 * dot( r, r );
						 		if( d<F1 ) {
						 			F2 = F1;
					...
					float2 voronoihash11_g43( float2 p )
					{
					...
					float voronoi11_g43( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash11_g43( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
								float d = 0.5 * dot( r, r );
						 		if( d<F1 ) {
						 			F2 = F1;
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float time35_g43 = 0.0;
				float2 voronoiSmoothId35_g43 = 0;
				float2 temp_output_21_0_g43 = CentralUV998;
				float2 break2_g43 = abs( temp_output_21_0_g43 );
				float saferPower4_g43 = abs( break2_g43.x );
				float saferPower3_g43 = abs( break2_g43.y );
				float saferPower6_g43 = abs( ( pow( saferPower4_g43 , 2.0 ) + pow( saferPower3_g43 , 2.0 ) ) );
				float Scale1080 = ( _MainCloudScale * 0.1 );
				float2 temp_output_10_0_g43 = (( ( temp_output_21_0_g43 * ( pow( saferPower6_g43 , Spherize1078 ) * Flatness1076 ) ) + float2( 0.5,0.5 ) )*( 2.0 / ( Scale1080 * 1.5 ) ) + ( Wind1035 * float2( 0.5,0.5 ) ));
				float2 coords35_g43 = temp_output_10_0_g43 * 60.0;
				float2 id35_g43 = 0;
				float2 uv35_g43 = 0;
				float fade35_g43 = 0.5;
				float voroi35_g43 = 0;
				float rest35_g43 = 0;
				for( int it35_g43 = 0; it35_g43 <2; it35_g43++ ){
				voroi35_g43 += fade35_g43 * voronoi35_g43( coords35_g43, time35_g43, id35_g43, uv35_g43, 0,voronoiSmoothId35_g43 );
				rest35_g43 += fade35_g43;
				coords35_g43 *= 2;
				fade35_g43 *= 0.5;
				}//Voronoi35_g43
				voroi35_g43 /= rest35_g43;
				float time13_g43 = 0.0;
				float2 voronoiSmoothId13_g43 = 0;
				float2 coords13_g43 = temp_output_10_0_g43 * 25.0;
				float2 id13_g43 = 0;
				float2 uv13_g43 = 0;
				float fade13_g43 = 0.5;
				float voroi13_g43 = 0;
				float rest13_g43 = 0;
				for( int it13_g43 = 0; it13_g43 <2; it13_g43++ ){
				voroi13_g43 += fade13_g43 * voronoi13_g43( coords13_g43, time13_g43, id13_g43, uv13_g43, 0,voronoiSmoothId13_g43 );
				rest13_g43 += fade13_g43;
				coords13_g43 *= 2;
				fade13_g43 *= 0.5;
				}//Voronoi13_g43
				voroi13_g43 /= rest13_g43;
				float time11_g43 = 17.23;
				float2 voronoiSmoothId11_g43 = 0;
				float2 coords11_g43 = temp_output_10_0_g43 * 9.0;
				float2 id11_g43 = 0;
				float2 uv11_g43 = 0;
				float fade11_g43 = 0.5;
				float voroi11_g43 = 0;
				float rest11_g43 = 0;
				for( int it11_g43 = 0; it11_g43 <2; it11_g43++ ){
				voroi11_g43 += fade11_g43 * voronoi11_g43( coords11_g43, time11_g43, id11_g43, uv11_g43, 0,voronoiSmoothId11_g43 );
				rest11_g43 += fade11_g43;
				coords11_g43 *= 2;
				fade11_g43 *= 0.5;
				}//Voronoi11_g43
				voroi11_g43 /= rest11_g43;
				float lerpResult15_g43 = lerp( saturate( ( voroi35_g43 + voroi13_g43 ) ) , voroi11_g43 , ( ModifiedCohesion1074 * 1.1 ));
				float lerpResult16_g43 = lerp( lerpResult15_g43 , 1.0 , ( ( 1.0 - CumulusCoverage376 ) + -0.7 ));
				float temp_output_1183_0 = saturate( (0.0 + (( Dot1071 * ( 1.0 - lerpResult16_g43 ) ) - 0.6) * (1.0 - 0.0) / (1.0 - 0.6)) );
...
/*ASEBEGIN
Version=18935
0;1080;2194.286;607.5715;1730.656;635.6709;1;True;False
...
Node;AmplifyShaderEditor.FunctionNode;1178;-763.9213,-2272.838;Inherit;True;Ghibli Clouds;-1;;43;bce7362c867d47d49a15818b7e6650d4;0;7;37;FLOAT2;0,0;False;21;FLOAT2;0,0;False;19;FLOAT;1;False;20;FLOAT;1;False;23;FLOAT;1;False;24;FLOAT;0;False;27;FLOAT;0.5;False;2;FLOAT;33;FLOAT;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;1231;-406.0242,-441.7039;Float;False;True;-1;2;;0;11;Distant Lands/Cozy/Stylized Clouds Ghibli Mobile;2992e84f91cbeb14eab234972e07ea9d;True;Forward;0;1;Forward;8;False;False;False;False;False;False;False;False;False;False;False;False;True;0;False;-1;False;True;1;False;-1;False;False;False;False;False;False;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;False;False;False;True;3;RenderPipeline=UniversalPipeline;RenderType=Opaque=RenderType;Queue=Transparent=Queue=0;True;2;True;17;d3d9;d3d11;glcore;gles;gles3;metal;vulkan;xbox360;xboxone;xboxseries;ps4;playstation;psp2;n3ds;wiiu;switch;nomrt;0;False;True;1;1;False;-1;0;False;-1;1;1;False;-1;0;False;-1;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;True;True;True;0;False;-1;False;False;False;False;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;True;1;False;-1;True;3;False;-1;True;True;0;False;-1;0;False;-1;True;1;LightMode=UniversalForward;False;False;0;Hidden/InternalErrorShader;0;0;Standard;22;Surface;0;0;  Blend;0;0;Two Sided;2;637952267559032767;Cast Shadows;1;0;  Use Shadow Threshold;0;0;Receive Shadows;1;0;GPU Instancing;1;0;LOD CrossFade;0;0;Built-in Fog;0;0;DOTS Instancing;0;0;Meta Pass;0;0;Extra Pre Pass;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,-1;0;  Type;0;0;  Tess;16,False,-1;0;  Min;10,False,-1;0;  Max;25,False,-1;0;  Edge Length;16,False,-1;0;  Max Displacement;25,False,-1;0;Vertex Position,InvertActionOnDeselection;1;0;0;5;False;True;True;True;False;False;;False;0
...
ASEEND*/
//CHKSM=F210481EB1F11E634AF78218659CFA9DA1E609D1